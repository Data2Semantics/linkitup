...
def link_to_sameas(*args, **kwargs):
    ...
    app.logger.debug("Running sameAs.org plugin for article {}".format(
        article_id))
    ...
    for item in match_items:
        ...
        sameas_links = [uri for h in hits for uri in h['duplicates']
                        if uri != item_link]

        for uri in sameas_links:
            # Create the match dictionary
            concept_label = label(uri, fallback=True)
            match = {
                'type': 'mapping',
                'uri': uri,
                'web': uri,
